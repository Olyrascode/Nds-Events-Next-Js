...
export default function Checkout() {
  ...
  const handleNext = () => {
    ...
    if (activeStep === 0 && deliveryMethod === 'delivery' && (!validateShippingInfo() || shippingFee === null)) {
      setError('Veuillez remplir toutes les informations de livraison et calculer les frais.');
      return;
  ...
  const handlePaymentSuccess = async (paymentIntent: PaymentIntent) => {
    ...
    try {
      ...
      const finalTotal = deliveryMethod === 'delivery' 
  ...
  const getStepContent = (step: number) => {
    switch (step) {
      case 0:
        return (
          <Box>
            <OrderSummary cart={cart} deliveryMethod={deliveryMethod} shippingFee={shippingFee} />
            <DeliveryOptions 
        ...
        <Box sx={{ display: 'flex', justifyContent: 'flex-end', mt: 3 }}>
          ...
          {activeStep < steps.length - 1 && (
            <Button 
              ...
              disabled={activeStep === 0 && deliveryMethod === 'delivery' && shippingFee === null}
            >
              Suivant
            </Button>
          )}
