...
async function fetchProducts(): Promise<Product[]> {
  ...
  const packs: Product[] = packsData
    .filter(
      (pack) =>
        // Filtrer les packs qui ont "decorations" dans leurs navCategories
        pack.navCategories && pack.navCategories.includes("decorations")
    )
    .map((pack) => ({
      _id: pack._id,
      id: pack._id,
  ...
  products.forEach((product) => {
    if (product.associations && product.associations.length > 0) {
      product.associations.forEach((assoc) => {
        if (assoc.navCategorySlug === "decorations" && assoc.categoryName) {
...
export async function generateMetadata(context: {
  params: Promise<{ category: string }>; // MAJ Promise
}) {
...
export default async function CategoryPage({
  ...
  const navCategory = "decorations"; // Spécifique à cette page
  const decodedCategory = decodeURIComponent(resolvedCategory);

  const products: Product[] = await fetchProducts();

  // Extraction des catégories uniques pour le filtre, spécifiques à "decorations"
